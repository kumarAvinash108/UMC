...
export function validateItemUpload(
  body: unknown,
): { ok: true; value: UploadShape } | { ok: false; error: string } {
  ...
  if (
    ...
    b.expires_at !== null &&
    isNaN(Date.parse(String(b.expires_at)))
  )
    return err("expires_at must be ISO date or null");
  // created_at is AAD-bound at encrypt time, so the server MUST preserve a
  // client-supplied value verbatim (else cross-device decrypt fails). Absent
  // (older clients) => server stamps now().
  if (
    b.created_at !== undefined &&
    (typeof b.created_at !== "string" || isNaN(Date.parse(b.created_at)))
  )
    return err("created_at must be an ISO date string");
  return {
    ok: true,
    value: {
      id: b.id as string,
      ...
      created_at: (b.created_at ?? null) as string | null,
    },
  };
}
...
export interface UploadShape {
  ...
  /** Client timestamp (AAD-bound). Null => server stamps now(). */
...
export function compareItems(
  a: Pick<ClipboardItem, "created_at" | "id">,
  b: Pick<ClipboardItem, "created_at" | "id">,
): number {
...
export function shouldApplyRemoteItem(
  ...
  item: Pick<ClipboardItem, "source_device_id" | "id">,
  seenIds: Set<string>,
): boolean {

...
describe("sync service v1", () => {
  ...
  it("E2E: upload ciphertext, other device receives it, server never sees plaintext", async () => {
    const s1 = (await api("/auth/session", { method: "POST", body: "{}" })).body as {
      user_id: string;
      token: string;
    };
    // device A registers
    const dA = (
      await api("/devices", {
        method: "POST",
        token: s1.token,
        body: JSON.stringify({ name: "ubuntu", platform: "linux", public_key: "pk-a" }),
      })
    ).body as { id: string };
    // second session for same user (device B flow): reuse user_id
    const s2 = (
      await api("/auth/session", { method: "POST", body: JSON.stringify({ user_id: s1.user_id }) })
    ).body as { user_id: string; token: string };
    const dB = (
      await api("/devices", {
        method: "POST",
        token: s2.token,
        body: JSON.stringify({ name: "phone", platform: "android", public_key: "pk-b" }),
      })
    ).body as { id: string };
  ...
  it("cursor pagination, idempotent retry, ack, delete propagation", async () => {
    const s = (await api("/auth/session", { method: "POST", body: "{}" })).body as {
      token: string;
      user_id: string;
    };
    await api("/devices", {
      method: "POST",
      token: s.token,
      body: JSON.stringify({ name: "fedora", platform: "linux", public_key: "pk" }),
    });
    const ids: string[] = [];
    for (let i = 0; i < 5; i++) {
      const id = randomUUID();
    ...
    const p1 = (await api("/items?limit=2", { token: s.token })).body as {
      items: { id: string }[];
      next_cursor: string;
    };
    ...
    const p2 = (
      await api(`/items?limit=2&cursor=${encodeURIComponent(p1.next_cursor)}`, { token: s.token })
    ).body as { items: unknown[]; next_cursor: string | null };
  ...
  it("revoked devices stop receiving; pairing short-code is single-use", async () => {
    ...
    const d = (
      await api("/devices", {
        method: "POST",
        token: s.token,
        body: JSON.stringify({ name: "d1", platform: "linux", public_key: "pk1" }),
      })
    ).body as { id: string };
    const pr = (
      await api("/pairing/request", {
        method: "POST",
        token: s.token,
        body: JSON.stringify({
          requester_public_key: "pk2",
    ...
    const cf1 = await api("/pairing/confirm", {
      method: "POST",
      token: s.token,
      body: JSON.stringify({ code: pr.code }),
    });
    assert.equal(cf1.res.status, 201);
    const cf2 = await api("/pairing/confirm", {
      method: "POST",
      token: s.token,
      body: JSON.stringify({ code: pr.code }),
    });
    ...
    const d = (
      await api("/devices", {
        ...
        body: JSON.stringify({
          ...
          platform: "linux",
          public_key: "pk",
          capabilities: ["wifi-lan", "bluetooth", "nfc"],
        }),
      })
    ).body as { capabilities: string[] };
    assert.deepEqual(d.capabilities, ["wifi-lan", "bluetooth"]); // unknown filtered
    const list = (await api("/devices", { token: s.token })).body as {
      devices: { capabilities: string[] }[];
    };
    assert.deepEqual(list.devices[0].capabilities, ["wifi-lan", "bluetooth"]);
  });

  it("preserves client created_at verbatim (it is AAD-bound for E2E decrypt)", async () => {
    const s = (await api("/auth/session", { method: "POST", body: "{}" })).body as {
      token: string;
      user_id: string;
    };
    const dev = (
      await api("/devices", {
        method: "POST",
        token: s.token,
        body: JSON.stringify({ name: "d", platform: "linux", public_key: "pk" }),
      })
    ).body as { id: string };
    const id = randomUUID();
    const created_at = "2024-05-01T12:00:00.000Z";
    const aad = {
      id,
      owner_id: s.user_id,
    ...
    const enc = encryptText({ key, plaintext: "ts-bound", aad });
    const up = await api("/items", {
      method: "POST",
      token: s.token,
      body: JSON.stringify({
    ...
    assert.equal(up.res.status, 201);
    assert.equal((up.body as { created_at: string }).created_at, created_at);
    // A second device decrypts with the SAME created_at the encryptor used.
    const list = (await api("/items", { token: s.token })).body as {
      items: {
        id: string;
        ciphertext: string;
        nonce: string;
    ...
    const got = list.items.find((i) => i.id === id)!;
    const { decryptText } = await import("@ucm/protocol");
    assert.equal(
      decryptText({
        key,
        ciphertextB64: got.ciphertext,
        nonceB64: got.nonce,
        aad: {
          id: got.id,
          owner_id: got.owner_id,
          source_device_id: got.source_device_id,
          content_type: "text/plain" as const,
